...
- **Codex's global `config.toml` is shared with the Codex app.** Synced from `$HOME` or into a profile, AgentSync owns only the keys `.ai/src` declares, settings plus one `mcp_servers.<id>` per MCP server, and keeps what the app writes there: project trust, hook state, plugins, its own MCP servers. A declared key the app changed stops the sync by name; `agentsync adopt .codex/config.toml` pulls it into the source. `targets.settings.ownership` (`auto`, `keys`, `file`) overrides the choice.
