...
- Choose an event transformer intentionally to prevent state collision and race conditions:
  - **sequential (default/preferred)** for strict ordering, ensuring BLoC processes events one at a time.
...
- When the UI needs to distinguish _why_ a success state was reached, store `lastEvent` in the state to allow listeners to differentiate reasons.
