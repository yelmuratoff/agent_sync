#!/usr/bin/env bash
# Installs git hooks to run lib/sync.sh after checkout and merge.
...
append_agentsync_block() {
...

    {
        ...
        echo "$BLOCK_START"
        cat <<'EOF'
if [ -f "lib/sync.sh" ]; then
    echo "Running AI Config Sync (lib)..."
    bash lib/sync.sh
elif [ -f "agent/lib/sync.sh" ]; then
    echo "Running AI Config Sync (agent/lib)..."
    bash agent/lib/sync.sh
elif command -v dart >/dev/null 2>&1; then
    echo "Running AI Config Sync (dart wrapper)..."
    dart run agent_sync:agent_sync sync --project-dir .
else
    echo "AgentSync: no sync entrypoint found (lib, agent/lib, or dart)." >&2
fi
EOF
        echo "$BLOCK_END"
    } >> "$hook_file"
