...
- **Shipped templates no longer pin a model — they inherit the account default:** the Claude `settings.json` template (`lib/templates/settings/claude.json`) and the subagent scaffolds (`lib/templates/agents/code-reviewer.md`, `lib/templates/content/subagent.md`, and the `agentsync generate` prompt) drop the hardcoded `model: sonnet`/`opus` field entirely. Claude Code expresses "use the recommended default model" by **omitting** `model` (the `/model` "Default" option clears the key), not by the literal string `"model": "default"` — that literal is read as a nonexistent custom model and errors (e.g. on `/compact`). New projects scaffolded by `agentsync init` — and existing ones picking up template content via `agentsync refresh` — now inherit the account's recommended model rather than a pinned snapshot, so a Claude Code / account-tier upgrade flows through without editing per-project config. Projects that deliberately pin a model through a `.ai/src/tools/<tool>/` override are unaffected; the skill's cost-saving guidance to pin `sonnet`/`haiku` for focused subagents still stands as an opt-in.
